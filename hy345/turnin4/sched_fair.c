...
enqueue_task_fair(struct rq *rq, struct task_struct *p, int flags)
{
	...
	check_preempt_wakeup(rq, p, flags);
	hrtick_update(rq);
}
...
static void check_preempt_wakeup(struct rq *rq, struct task_struct *p, int wake_flags)
{
	...
	/**/
	long curr_value, p_value;
	curr_value = get_score(curr);
        p_value = get_score(p);
	/**/
	if (unlikely(se == pse))
		return;
	...
	if (sched_feat(NEXT_BUDDY) && scale && !(wake_flags & WF_FORK))
		set_next_buddy(pse);
	/**/
	if (p_value > curr_value) {
        printk("preemp task pid=%d value=%ld, with pid=%d,value=%ld\n",curr->pid, curr_value, p->pid, p_value);
        goto preempt;
        }
	/*
